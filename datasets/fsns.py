...
DEFAULT_CONFIG = {
    ...
    'charset_filename': 'dic.txt',#charset_size=134.txt',
...
def TFRecordToImg():
    ...
    config = DEFAULT_CONFIG
    charset_file = os.path.join("data/fsns", config['charset_filename'])
    charset = read_charset(charset_file)
    with tf.Session() as sess:
        ...
        for i in range(10):
            ...
            fn='data/img/' + str(i) +'.jpg'
            # print(fn,img.shape)
            img = Image.fromarray(img, 'RGB')  # 这里将narray转为Image类，Image转narray：a=np.array(img)
            img.save(fn)#
            strOut=[]
            strOutLb.append([fn,lab])

            print(lab)
            for i in lab:
                if i<133:
                    strOut.append(charset[i])
            # print(fn,strOut)
        # for lb in strOutLb:
        #     print(lb)
        df = pd.DataFrame(strOutLb)
        df.to_csv("data/imgLb.csv",sep=",",header=None,index=None)
...
def encode_utf8_string(text, length, charset, null_char_id=133):
    ...
    char_ids_padded_int = [null_char_id]*length
    char_ids_unpadded_int = [null_char_id]*len(text)
    for i in range(len(text)):
        hash_id = charset[text[i]]
        char_ids_padded[i] = hash_id
        char_ids_unpadded[i] = hash_id
        char_ids_padded_int[i] = text[i]
        char_ids_unpadded_int[i] = text[i]
    return char_ids_padded, char_ids_unpadded,char_ids_padded_int,char_ids_unpadded_int
